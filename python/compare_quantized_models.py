...
basePath = "../SantanderCreateML/SantanderCreateML/"

test = pd.read_csv(basePath + "Data/train_head_pre.csv")
...
test_coreml["prediction"] = test_coreml["target"]
print(test_coreml.describe())

models = ["SantanderBoostedTreeRegressor_100_000_it", "SantanderBoostedTreeRegressor_20_000_it", "SantanderBoostedTreeRegressor_2_500_it"]
for idx, m in enumerate(models):
    print("Comparing non-quantized vs quantized version of model {0}".format(m))

    non_quantized_model = MLModel(basePath + m + ".mlmodel")
    metrics = evaluate_regressor(non_quantized_model, test_coreml, target="target", verbose=False)
    print("non-quantized model metrics")
    print(metrics)

    quantized_model = MLModel(basePath + m + "Q.mlmodel")
    metrics = evaluate_regressor(quantized_model, test_coreml, target="target", verbose=False)
