...
from coremltools.models.utils import evaluate_regressor, convert_neural_network_weights_to_fp16
from coremltools.models.neural_network.quantization_utils import compare_models
from coremltools.models import MLModel

# test = pd.read_csv("../data/train_head.csv")
...
# sample_data = test_coreml.to_dict(orient="records")
# print(sample_data[0])

#models = ["XgbRegressor{0}.mlmodel".format(i) for i in range(1,4)]
#basePath = "./Models/"
basePath = "../SantanderCreateML/SantanderCreateML/Models/"
models = ["SantanderBoostedTreeRegressor_2_500_it", "SantanderBoostedTreeRegressor_20_000_it", "SantanderBoostedTreeRegressor_100_000_it"]
for idx, model_name in enumerate(models):
    model = MLModel(basePath + model_name + ".mlmodel")
    model_type = model.get_spec().WhichOneof('Type')
    model_pipeline_type = model.get_spec().pipeline
    print("Quantizing {0} with type {1} and pipeline {2}".format(model_name, model_type, model_pipeline_type))
    quantized_model = convert_neural_network_weights_to_fp16(model)
    # print("compare non-quantized and quantized versions")
    # compare_models(model, quantized_model, sample_data) # won't work: Unable to determine which metric to compute for output: target
    quantized_model.save(basePath + model_name + "Q.mlmodel")
